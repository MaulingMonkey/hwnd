...
/// # use std::ptr::*;
/// unsafe {
/// #   let thread_local_hwnd = create_window_w(abistr::utf16ish!("Message"), (), 0, 0, 0, 0, 0, HWnd::MESSAGE, null_mut(), None, null_mut()).unwrap();
///     send_message_w(thread_local_hwnd,    WM::NULL, 0, 0).unwrap();
/// #   send_message_w(thread_local_hwnd,    WM::SETTEXT, 0, abistr::utf16ish!("asdf").as_ptr() as _).unwrap(); // IDK if this is the right string width or not
...
/// # use std::ptr::*;
/// # use std::sync::atomic::*;
/// # let thread_local_hwnd = unsafe { create_window_w(abistr::utf16ish!("Message"), (), 0, 0, 0, 0, 0, HWnd::MESSAGE, null_mut(), None, null_mut()).unwrap() };
/// unsafe {
...
///             abistr::utf16ish!("asdf").as_ptr() as _, // *guessing* that WM_SETTEXT takes _TCHAR[]
///             never_called, 0,
///         ).unwrap_err()
///     );
///
...
/// unsafe {
/// #   let thread_local_hwnd = create_window_w(abistr::utf16ish!("Message"), (), 0, 0, 0, 0, 0, HWnd::MESSAGE, null_mut(), None, null_mut()).unwrap();
/// #   send_message_timeout_w(thread_local_hwnd,    WM::SETTEXT, 0, abistr::utf16ish!("asdf").as_ptr() as _, SMTO::NORMAL, 100).unwrap();
...
///     );
///
///     let text = abistr::utf16ish!("asdf").as_ptr(); // *guessing* WM::SETTEXT takes TCHAR[]
///     assert_eq!(
///         ERROR::MESSAGE_SYNC_ONLY,
